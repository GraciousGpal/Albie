...
class Market(commands.Cog):
	...
	async def prices(self, ctx, *, item=None):
		...
		async with ctx.channel.typing():
			...
			current_data, history_data = await asyncio.gather(self.get_current_data(item_name),
															  self.get_history_data(item_name))
			...
			# Get avg_stats
			average_sell_price, avg_sell_volume, best_city_to_sell = get_avg_stats(h_data)

			best_cs_str = f'{best_city_to_sell[0]} ({c_game_currency(best_city_to_sell[1])})'

			# Start embed object
			title = f"{item_f[0][1]['LocalizedNames']['EN-US']} (Enchant:{enchant})\n"
			buyorder_embed = Embed(color=0x98FB98, title=title,
			...
			if len(current_prices[0].index) != 0:
				...
				if len(normalcheck_s) != 0:
					avg_s_current_price = [x for x in current_prices[0].loc[normalcheck_s[0], :] if not math.isnan(x)]
					avg_s_cp = c_game_currency(int(average(avg_s_current_price)))
					buyorder_embed.add_field(name=f"Avg Current Sell Price ({normalcheck_s[0]})", value=avg_s_cp,
											 inline=True)

			if len(current_prices[2].index) != 0:
				normalcheck_b = [str(x) for x in current_prices[2].T]
				if 'Normal' in normalcheck_b:
					avg_b_current_price = [x for x in current_prices[2].loc[normalcheck_b[0], :] if not math.isnan(x)]
			...
			if history_buffer is not None:
				...
				history_embed.add_field(name="Avg Historical Price (Normal)", value=average_sell_price, inline=True)
				history_embed.add_field(name="Avg Sell Volume", value=avg_sell_volume, inline=True)
	...
	async def c_price_table(self, cdata):
		...
		for city in cdata:
			...
			city_table_last_updated[city['city']][city['quality'] - 1] = city['sell_price_min_date']
			city_buy_table_last_updated[city['city']][city['quality'] - 1] = city['buy_price_min_date']
		...
		city_table_last_updated = pd.DataFrame(city_table_last_updated, index=self.quality_tiers)
		city_buy_table_last_updated = pd.DataFrame(city_buy_table_last_updated, index=self.quality_tiers)
		...
		city_table_last_updated = city_table_last_updated.apply(c_last_updated_series, axis=0)

		city_buy_table_last_updated = city_buy_table_last_updated.dropna(axis=0, how='all')
		city_buy_table_last_updated = city_buy_table_last_updated.dropna(axis=1, how='all')
		city_buy_table_last_updated = city_buy_table_last_updated.fillna('')
		city_buy_table_last_updated = city_buy_table_last_updated.apply(c_last_updated_series, axis=0)
		...
		if current_data[0].empty and current_data[2].empty:
			...
			for h_map in current_data:
				if (index_ % 2) == 0 or index_ == 0:
					...
					sb_map = sns.heatmap(h_map, annot=current_data[index_ + 1].to_numpy(), ax=ax[axx], fmt='',
										 cbar=False)
			...
			for h_map in current_data:
				if (index_ % 2) == 0 or index_ == 0:
					title = "Sell Order" if index_ == 0 else "Buy Order"
					if not h_map.empty:
						sb_map = sns.heatmap(h_map, annot=current_data[index_ + 1].to_numpy(), ax=ax, fmt='',
											 cbar=False)
						sb_map.set_xticklabels(sb_map.get_xticklabels(), rotation=30)
						sb_map.set_yticklabels(sb_map.get_yticklabels(), rotation=0)
						sb_map.set_title(title)

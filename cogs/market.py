...
class Market(commands.Cog):
    def __init__(self, client):
        ...
        try:
            ...
            self.dict = json.load(
                open("data/item_data.json", "r", encoding="utf-8"))
    ...
    async def prices(self, ctx, *, item=None):
        ...
        if item_w in self.id_list:
            item_f = [(11, item)
                      for item in self.dict if item["UniqueName"] == item_w]
        ...
        async with ctx.channel.typing():
            ...
            current_data, history_data = await asyncio.gather(
                self.get_current_data(
                    item_name), self.get_history_data(item_name)
            )
            ...
            if len(current_prices[0].index) != 0:
                ...
                if len(normalcheck_s) != 0:
                    ...
                    avg_s_cp = c_game_currency(
                        int(average(avg_s_current_price)))
            ...
            if len(current_prices[2].index) != 0:
                ...
                if "Normal" in normalcheck_b:
                    ...
                    avg_b_cp = c_game_currency(
                        int(average(avg_b_current_price)))
                    buyorder_embed.add_field(
            ...
            filename = (
                f'{item_name.replace("@", "_")}_{datetime.today().strftime("%Y_%m_%d")}'
            )
            ...
            if history_buffer is not None:
                ...
                history_file = File(
                    history_buffer, filename=f"{filename}H.png")
    ...
    async def get_current_data(self, item_name):
        try:
            currurl = (
                ...
                + "".join(
                    ["," + "".join(x)
                     for x in self.locations if x != self.locations[0]]
                )
            )
    ...
    def search_processing(self, condition, item_w):
        ...
        if not condition:
            ...
            list_v = [
                s for s in list_v if not "NONTRADABLE" in s["UniqueName"]
            ]  # Remove non tradable items
            if tier is not None:
                list_v = [
                    x for x in list_v if f"T{tier[0]}" in x["UniqueName"]]
                item_w = item_w.replace(f"T{tier[0]}", "")
            if enchant is not None:
                list_v = [
                    x for x in list_v if f"@{enchant}" in x["UniqueName"]]
    ...
    async def c_price_table(self, cdata):
        ...
        for city in cdata:
            ...
            city_table[city["city"]][city["quality"] -
                                     1] = city["sell_price_min"]
        ...
        city_table_last_updated = city_table_last_updated.dropna(
            axis=0, how="all")
        city_table_last_updated = city_table_last_updated.dropna(
            axis=1, how="all")
        ...
        sell_annotation = sell_annotation.add(
            city_table_last_updated, fill_value="")

        # buy orders
        buy_annotation = buy_data.apply(c_game_series, axis=0)
        buy_annotation = buy_annotation.fillna("")
        buy_annotation = buy_annotation.add(
            city_buy_table_last_updated, fill_value="")
        ...
        if current_data[0].empty and current_data[2].empty:
            ...
            for h_map in current_data:
                if (index_ % 2) == 0 or index_ == 0:
                    ...
                    sb_map.set_xticklabels(
                        sb_map.get_xticklabels(), rotation=30)
                    sb_map.set_yticklabels(
                        sb_map.get_yticklabels(), rotation=0)
    ...
    async def full_graph(self, cdata):
        ...
        for city in data:
            ...
            if a["avg_price"].size < 10:
                ...
                w_data[city] = a[
                    a["avg_price"].between(
                        a["avg_price"].quantile(
                            0.1), a["avg_price"].quantile(0.9)
                    )
                ]
